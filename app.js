// App: login demo + gestionale CRUD (localStorage)
(function(){
  // ===== Utilities =====
  ...
  const loginForm=$('#loginForm'), boxStatus=$('#formStatus');
  const togglePwd=$('#togglePwd');
  const gestione=$('#gestionale'), navGest=$('#navGestionale');
  const logoutBtn=$('#logoutBtn');

  function setAuthUI(isAuth){
    if (gestione) gestione.hidden = !isAuth;
    if (navGest) navGest.style.opacity = isAuth ? '1' : '.6';
  }
  function isLoggedIn(){ return localStorage.getItem('pa_logged_in')==='1'; }
  function loginPersist(remember){
    localStorage.setItem('pa_logged_in','1');
  ...
  loginForm?.addEventListener('submit', (e)=>{
    ...
    if(!ok){
      boxStatus.hidden=false; boxStatus.textContent='Credenziali non valide. Usa admin@demo.it / Demo123!';
      return;
    }
    boxStatus.hidden=true; boxStatus.textContent='';
    loginPersist(remember); setAuthUI(true); location.hash='#gestionale';
  });
  ...
  // Filters & search
  $('#filterReqStatus')?.addEventListener('change', renderRichieste);
  $('#filterReqDate')?.addEventListener('change', renderRichieste);
  $('#filterShiftDate')?.addEventListener('change', renderTurni);
  $('#filterVehicleState')?.addEventListener('change', renderMezzi);
  $('#filterVolunteer')?.addEventListener('input', renderVolontari);
  $('#globalSearch')?.addEventListener('input', ()=>{ renderRichieste(); renderTurni(); renderMezzi(); renderVolontari(); });
  ...
  // Init gestionale UI on load if logged in
  rerender();
})();